...
def resize_with_pad_v2(images, height: int, width: int, method=Image.BILINEAR):
...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.language_tokenizer = AutoTokenizer.from_pretrained("google/paligemma-3b-pt-224", local_files_ony=True)
        ...
        self.temporal_ensembler = TemporalEnsembler(0.015, 16)
    ...
    def select_action_chunk(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        action_key = list(self.config.output_features.keys())[0]
    ...
    def forward(self, batch: dict[str, Tensor], noise=None, time=None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        # Remove padding
        # losses = losses[:, :, : self.config.max_action_dim] #TODO: it should be self.config.action_feature.shape[0]
    ...
    def prepare_images_jax(self, batch):
        """
        Using padding exaclty like Jax, the above habe some margin of erro, also add blank image as well (but it seems no difference)
        Apply Pi0 preprocessing to the images, like resizing to 224x224 and padding to keep aspect ratio, and
        convert pixel range from [0.0, 1.0] to [-1.0, 1.0] as requested by SigLIP.
        """
        images = []
        ...
        present_img_keys = [key for key in self.config.image_features if key in batch and "depth" not in key and "mask" not in key]
        missing_img_keys = [key for key in self.config.image_features if key not in batch]
        if not missing_img_keys:
            missing_img_keys = ["blank"]
        if len(present_img_keys) == 0:
            raise ValueError(
                f"All image features are missing from the batch. At least one expected. (batch: {batch.keys()}) (image_features:{self.config.image_features})"
            )
        ...
        for key in present_img_keys:
            ...
            if self.config.resize_imgs_with_padding is not None:
                img = resize_with_pad_v2(img, *self.config.resize_imgs_with_padding)
            # Normalize from range [0,1] to [-1,1] as expected by siglip
...
class PI0FlowMatching(nn.Module):
    ...
    def forward(
        ...
        # print("prefix emb:", prefix_embs.shape)
    ...
    def sample_actions(self, images, img_masks, lang_tokens, lang_masks, state, noise=None, **kwargs: Unpack[ActionSelectKwargs]) -> Tensor:
